...
const EditListingScreen = ({ route, navigation }) => {
  ...
  const [selectedAvailableFromDate, setSelectedAvailableFromDate] = useState(
    {}
  );
    ...
    api
      ...
      .then((res) => {
        if (res.ok) {
          ...
          if (config?.booking && res?.data?.config?.booking) {
            setBookingForm(res.data.config.booking);
            autoUpdateBookingType(
              res.data.config.booking.type,
              res.data.listing.booking._rtcl_listing_booking
            );
            if (res?.data?.listing?.booking) {
              if (res.data.listing.booking._rtcl_shs) {
                const tempSHS = { ...res.data.listing.booking._rtcl_shs };
                const tempBookingData = { ...res.data.listing.booking };
                delete tempBookingData._rtcl_shs;
                setBookingData(tempBookingData);
                setShs(tempSHS);
              } else {
                setBookingData(res.data.listing.booking);
              }
              if (res.data.listing.booking._rtcl_booking_disable_date) {
                const tempDateArr =
                  res.data.listing.booking._rtcl_booking_disable_date;
                let tempSelectedDays = {};
                tempDateArr.map((_tDA) => {
                  tempSelectedDays[_tDA] = {
                    selected: true,
                  };
                });
                // Object.keys(tempDateArr).map((_tDA) => {
                //   const _tDAVal = tempDateArr[_tDA];
                //   tempSelectedDays[_tDAVal] = {
                //     selected: true,
                //   };
                // });
                setSelectedDates(tempSelectedDays);
              }
              if (
                res.data.listing.booking._rtcl_booking_pre_order_available_date
              ) {
                setSelectedAvailableFromDate({
                  [res.data.listing.booking
                    ._rtcl_booking_pre_order_available_date]: {
                    selected: true,
                  },
                });
              }
  ...
  const handleUpdateListing = (values) => {
    ...
    const data = {
      manage: route.params.admin ? true : false,
...
`;
  ...
  const SHComponent = ({ day, dayName }) => (
    <View style={[styles.bHDayWrap, rtlView]}>
      ...
      >
        <TouchableOpacity
          style={[styles.openButtonWrap, rtlView]}
          onPress={() => handleSHDayOpenBtnPress(day)}
        >
          <MaterialCommunityIcons
            name={shs[day].open ? "checkbox-marked" : "checkbox-blank-outline"}
            size={24}
            color={COLORS.primary}
          />
        ...
        {shs[day].open && (
          <>
            <TouchableOpacity
              style={[styles.timeSlotToggleBtnWrap, rtlView]}
              onPress={() => handleSHTimeSlotToggleBtnPress(day)}
            >
              <MaterialCommunityIcons
                name={
                  !!shs[day]?.times
            ...
            {!!shs[day]?.times && (
              ...
              >
                {shs[day].times.map((_slot, index, arr) => (
                  <View style={[styles.timeSlot, rtlView]} key={index}>
                    <View
                      style={[
                        styles.timeSltStartWrap,
                    ...
                    >
                      ...
                      <View style={styles.slotTimeWrap}>
                        <BHTimePicker
                          ...
                          type="start"
                          day={day}
                          onSelectTime={handleSHTimePickerEvent}
                          serial={index}
                          is12hr={
                            config.store?.time_options?.showMeridian ?? true
                          }
                    ...
                    <View style={[styles.btnWrap, rtlView]}>
                      {arr.length > 1 && (
                        <TouchableOpacity
                          style={[styles.sltDltBtn, { flex: 0.5 }]}
                          onPress={() => handleSHTimeSltDlt(day, index)}
                        >
                          <FontAwesome
                            name="minus-circle"
                            size={20}
                      ...
                      {index === arr.length - 1 && (
                        <TouchableOpacity
                          style={[styles.sltAddBtn, { flex: 0.5 }]}
                          onPress={() => handleSHTimeSltAdd(day, index)}
                        >
                          <FontAwesome
                            name="plus-circle"
                            size={20}
  ...
  const handleSHDayOpenBtnPress = (day) => {
    const tempSHDay = { ...shs[day], open: !shs[day].open };
    if (!tempSHDay.open) {
      delete tempSHDay.times;
    }
    setShs({ ...shs, [day]: tempSHDay });
  };

  const handleSHTimeSlotToggleBtnPress = (day) => {
    const format = config.datetime_fmt.time || "h:mm a";
    const tempTimeSlot = {
      start: moment("8:00 am", "h:mm a").format(format),
      end: moment("8:00 pm", "h:mm a").format(format),
    };
    let tempSHDay = { ...shs[day] };
    if (!!tempSHDay?.times) {
      delete tempSHDay.times;
    } else {
      tempSHDay["times"] = [tempTimeSlot];
    }
    setShs({ ...shs, [day]: tempSHDay });
  };

  const handleSHTimeSltAdd = (day) => {
    const format = config.datetime_fmt.time || "h:mm a";
    const tempTimeSlot = {
      start: moment("8:00 am", "h:mm a").format(format),
      end: moment("8:00 pm", "h:mm a").format(format),
    ...
    const tempSHDay = {
      ...shs[day],
      times: [...shs[day].times, tempTimeSlot],
    };
    const tempSH = { ...shs, [day]: tempSHDay };
    setShs(tempSH);
  };

  const handleSHTimeSltDlt = (day, index) => {
    const tempTimes = shs[day].times.filter(
      (_timeSlots, _index) => _index !== index
    );
  ...
  const handleSHTimePickerEvent = (day, type, payload, serial) => {
    const format = config.datetime_fmt.time || "h:mm a";

    let tempSHDay = { ...shs[day] };
    let tempTimeSlts = [...shs[day].times];
    let temptimeSlt = { ...shs[day].times[serial] };
    ...
    tempTimeSlts[serial] = temptimeSlt;
    tempSHDay["times"] = tempTimeSlts;

    setShs({ ...shs, [day]: tempSHDay });
  };

  const autoUpdateBookingType = (type, preType) => {
    if (preType) {
      setBookingData((prevBookingData) => {
        return { ...prevBookingData, [type.name]: preType };
      });
      setBookingType(preType);
      return;
    }
    const activeTypes = Object.keys(type.options).filter((_type) => {
      consttypeObj = config.booking[_type];
      return consttypeObj?.disable === false;
    });
    if (activeTypes.length) {
      setBookingData((prevBookingData) => {
        return { ...prevBookingData, [type.name]: activeTypes[0] };
      });
      setBookingType(activeTypes[0]);
    }
  };

  const handleBookingDataChange = (field, value) => {
    setBookingData((prevBookingData) => {
      return { ...prevBookingData, [field]: value };
    });
    if (field === bookingForm.type.name) {
  ...
  const handleSelectedDay = (day, fieldName) => {
    let tempArr = bookingData[fieldName] || [];
    if (tempArr.includes(day.dateString)) {
      const tempNewArr = tempArr.filter((_tnad) => _tnad != day.dateString);
      setBookingData((prevData) => {
        return {
          ...prevData,
          [fieldName]: tempNewArr,
        };
      });
      const tempSelectedDates = { ...selectedDates };
      delete tempSelectedDates[day.dateString];
      setSelectedDates(tempSelectedDates);
    } else {
      setBookingData((prevData) => {
        return {
          ...prevData,
          [fieldName]: [...tempArr, day.dateString],
        };
      });
      setSelectedDates((prevDays) => {
        return {
          ...prevDays,
          [day.dateString]: {
            selected: true,
  ...
  const handleAvailableFromDate = (day, fieldName) => {
    setBookingData((prevData) => {
      return {
        ...prevData,
        [fieldName]: day.dateString,
      };
    });
    setSelectedAvailableFromDate({
                  ...
                  <Formik
                    initialValues={{
                      ...
                      description: listingData?.listing?.description
                        ...
                        : "",
                      name: route?.params?.admin
                        ? getUserName(listingData?.listing?.author || "")
                        : getUserName(user) || "",
                      zipcode: route?.params?.admin
                        ? listingData?.listing?.contact?.zipcode || ""
                        : listingData?.listing?.contact?.zipcode ||
                          user?.zipcode ||
                          "",
                      address: route?.params?.admin
                        ? decodeString(
                            listingData?.listing?.contact?.address || ""
                          ) ||
                          decodeString(
                            listingData?.listing?.author?.address || ""
                          ) ||
                          ""
                        : decodeString(
                            listingData?.listing?.contact?.address || ""
                          ...
                          "",
                      phone: route?.params?.admin
                        ? listingData?.listing?.contact?.phone ||
                          listingData?.listing?.author?.phone ||
                          ""
                        : listingData?.listing?.contact?.phone ||
                          user?.phone ||
                          "",
                      whatsapp_number: route?.params?.admin
                        ? listingData?.listing?.contact?.whatsapp_number ||
                          listingData?.listing?.author?.whatsapp_number ||
                          ""
                        : listingData?.listing?.contact?.whatsapp_number ||
                          user?.whatsapp_number ||
                          "",
                      email: route?.params?.admin
                        ? listingData?.listing?.contact?.email ||
                          listingData?.listing?.author?.email ||
                          ""
                        : listingData?.listing?.contact?.email ||
                          user?.email ||
                          "",
                      website: route?.params?.admin
                        ? listingData?.listing?.contact?.website ||
                          listingData?.listing?.author?.website ||
                          ""
                        : listingData?.listing?.contact?.website ||
                          user?.website ||
                          "",
                            ...
                            <View style={styles.bHWrap}>
                              ...
                              <View style={styles.bHContentWrap}>
                                <TouchableOpacity
                                  ...
                                  onPress={() =>
                                    handleBookingDataChange(
                                      bookingForm.active.name,
                                      bookingData[bookingForm.active.name]
                                        ? false
                                        : true
                                    )
                                  }
                                >
                                  ...
                                  >
                                    {bookingForm?.active?.label ||
                                      __(
                                        "listingFormTexts.bookingToggleTitle",
                                        appSettings.lng
                                      ) ||
                                      ""}
                                  </Text>
                                ...
                                {bookingData[bookingForm.active.name] && (
                                  <>
                                    <View
                                      style={[styles.bookingTypeWrap, rtlView]}
                                    >
                                      <View style={styles.bookingTypeTitleWrap}>
                                        <Text
                                          style={[
                                        ...
                                        >
                                          {bookingForm?.type?.label ||
                                            __(
                                              "listingFormTexts.bookingTypeTitle",
                                              appSettings.lng
                                            ) ||
                                            ""}
                                        </Text>
                                      ...
                                      <View
                                        style={[
                                          styles.bookingTypesWrap,
                                          {
                                            justifyContent: rtl_support
                                              ? "flex-end"
                                              : "flex-start",
                                            marginTop: 5,
                                          },
                                      ...
                                      >
                                        {Object.keys(
                                          bookingForm.type.options
                                        ).map((_btype, index) => {
                                          if (
                                            config.booking[_btype].disable ===
                                            false
                                          ) {
                                            return (
                                              <Pressable
                                              ...
                                              >
                                                ...
                                                >
                                                  {bookingForm?.type?.options[
                                                    ...
                                                    __(
                                                      `listingFormTexts.bookingTypes.${_btype}`,
                                                      appSettings.lng
                                                    ) ||
                                                    ""}
                                                </Text>
                                    ...
                                    <View style={styles.bHs}>
                                      {bookingForm?.fee && (
                                        <View
                                          style={[styles.bHDayWrap, rtlView]}
                                        >
                                          <View
                                            style={{
                                              alignItems: rtl_support
                                      ...
                                      {!!bookingData[bookingForm.type.name] &&
                                        Object.keys(
                                          bookingForm[bookingType]
                                        ).map((_bookingField, index) => {
                                          var bookingField =
                                            bookingForm[bookingType][
                                              _bookingField
                                            ];
                                          var bookingFieldType =
                                            bookingField.type;
                                          if (bookingFieldType === "custom") {
                                            return (
                                              <View
                                                style={[
                                                  { marginVertical: 10 },
