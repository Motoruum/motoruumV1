...
const ChatScreen = ({ navigation, route }) => {
  const [{ user, auth_token, ios, appSettings, rtl_support, config }] =
    useStateValue();
  ...
  const parseTime = (time) => {
    ...
    if (config?.tz) {
      if (config.tz?.timezone_type == 1) {
        const rawTime = time.split(" ").join("T") + config.tz.timezone;
        resultTime = moment(rawTime).format("MMM Do, h:ma");
      } else if (config.tz?.timezone_type == 3) {
        resultTime = moment
          .tz(time, "YYYY-MM-DD HH:mm:ss", config.tz.timezone)
          .format("MMM Do, h:ma");
      }
    } else {
      resultTime = moment(time, "YYYY-MM-DD").fromNow();
    }
    return resultTime;
  };
    ...
    >
      ...
      >
        ...
        >
          {time
            ? parseTime(time) || ""
            : __("chatScreenTexts.now", appSettings.lng)}
        </Text>
        {sender && (
            ...
            >
              {({ handleChange, handleBlur, handleSubmit, values, errors }) => (
                <View style={styles.chatBoxWrap}>
                  ...
                  >
                    <SendIcon fillColor={COLORS.primary} />
                  </TouchableOpacity>
                </View>
              )}
            </Formik>
