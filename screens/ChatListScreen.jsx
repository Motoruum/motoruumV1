...
const ChatListScreen = ({ navigation, route }) => {
  ...
  const getTime = (time) => {
    ...
    if (config?.tz) {
      if (config.tz?.timezone_type == 1) {
        resultTime = moment.parseZone(time + config.tz.timezone).fromNow();
      } else if (config.tz?.timezone_type == 3) {
        resultTime = moment.tz(time, config.timezone.timezone).fromNow();
      }
    } else {
  ...
  const renderChats = ({ item }) => (
    <Chat
      ...
      time={item.last_message_created_at}
