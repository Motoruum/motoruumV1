...
const SelectLocationScreen = ({ route, navigation }) => {
  ...
  const flipHorizontally = rtl_support && {
    transform: [{ scaleX: -1 }],
  };
    ...
    if (location.length) {
      if (route?.params?.promotion) {
        if (route.params.promotion == "featured") {
          dispatch({
            type: "SET_FEATURED_SEARCH_LOCATIONS",
            featured_search_locations: location,
          });
        }
        if (route.params.promotion == "_top") {
          dispatch({
            type: "SET_TOP_SEARCH_LOCATIONS",
            top_search_locations: location,
          });
        }
      } else {
        dispatch({
