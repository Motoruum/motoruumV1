...
import StoreAdsCard from "../components/StoreAdsCard";
...
const StoreDetailsScreen = ({ route, navigation }) => {
  ...
  const renderListItem = useCallback(
    ({ item }) => <StoreAdsCard item={item} onClick={handleViewListing} />,
    []
  );
  const getTaxonomy = (data) => {
    if (!data?.length) return false;
    return decodeString(data[data.length - 1].name);
  };
