...
const NewListingScreen = ({ navigation }) => {
            ...
            {categories[arg].map((_category) => (
              ...
              >
                {!currentCategories?.length && (
                  ...
                  >
                    {_category?.icon?.url ? (
                      <CategoryImage size={20} uri={_category.icon.url} />
                    ) : (
                      <CategoryIcon
  ...
  const handleGoBackonSuccess = (data) => {
    // handleBackButtonClick();
    if (!!config?.redirect_new_listing) {
      if (config.redirect_new_listing === "home") {
        dispatch({
          type: "SET_NEW_LISTING_SCREEN",
          listing_locations: null,
        });
        navigation.replace(routes.drawerNavigator);
      } else if (config.redirect_new_listing === "my_ads") {
        dispatch({
          type: "SET_NEW_LISTING_SCREEN",
          listing_locations: null,
        });
        navigation.navigate(routes.myListingsScreen, {
          hidden: true,
        });
      } else if (
        config.redirect_new_listing === "promotion" &&
        (!config?.iap_disabled ||
